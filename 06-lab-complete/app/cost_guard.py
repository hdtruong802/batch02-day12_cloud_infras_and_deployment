"""Redis-backed monthly cost guard per user."""
...
def estimate_cost(input_tokens: int, output_tokens: int) -> float:
    return (input_tokens / 1000) * PRICE_PER_1K_INPUT_TOKENS + (
        output_tokens / 1000
...
def check_budget(user_id: str, estimated_cost: float) -> None:
    """
    Track monthly spending per user in Redis.
    Raises HTTP 402 when monthly budget exceeded.
    """
    r = get_redis()
